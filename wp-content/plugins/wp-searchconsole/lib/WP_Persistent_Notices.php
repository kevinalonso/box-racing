...
if (!class_exists('WP_Persistent_Notices')) {
    ...
    {  // replace ClassName with the name of your class
        ...
        public function add_notice($notice = array(), $location)
        {
            if (!is_array($notice)) {
                $notice = array(
                    'message' => $notice
        ...
        public function render_notices($location = 'default')
        {
            ...
            foreach ($notices as $notice) {
                ...
                if ($notice['html']) {
                    ...
                    $message = 'WP Search Console: ' . wp_kses_post($notice['message']);
        ...
        public function ajax_get_notices()
        {
            ...
            if (isset($_GET['location']) && $_GET['location']) {
                $location = esc_sql(sanitize_text_field($_GET['location']));
            }
            if (isset($_POST['location']) && $_POST['location']) {
                $location = esc_sql(sanitize_text_field($_POST['location']));
            }
            $notices = $this->retrieve_notices($location);
    ...
    if (!function_exists('add_persistent_notice')) {
        function add_persistent_notice($notice, $location = 'default')
        {
            $cl = WP_Persistent_Notices::Instance();
            return $cl->add_notice($notice, $location);
        }
    }
